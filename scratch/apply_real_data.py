...
# The correct real products list
PRODUCTS_CODE = """const PRODUCTS = [
    // --- PROMOZIONI (TRANSPALLET) ---
    {
        id: 'ech12c',
        name: 'STILL ECH 12 C',
...
function initForkliftFinder() {
    ...
    function renderProducts() {
        ...
        const filtered = PRODUCTS.filter(prod => {
            // Match tipologia
            const matchTipologia = activeFilters.tipologia === 'tutti' || 
                                   prod.specs.tipologia === activeFilters.tipologia;
            ...
            if (activeFilters.alimentazione !== 'tutti') {
                matchAlimentazione = prod.specs.alimentazione === activeFilters.alimentazione;
            }
        ...
        filtered.forEach(prod => {
            ...
            if (prod.specs.alimentazione) {
                let dispAlim = prod.specs.alimentazione;
                if (dispAlim === 'litio') dispAlim = 'Ioni di Litio';
                specsHtml += `<li>Alimentazione: <strong style="text-transform: capitalize;">${dispAlim}</strong></li>`;
            }
...
<label class="filter-btn">
<input name="tipologia" type="radio" value="stoccatori"/>
                                Stoccatori / A Colonna
                            </label>
<label class="filter-btn">
...
def update_js_files():
    ...
    for path in paths:
        ...
        # We find start of function initForkliftFinder() and replace it up to function initScrollAnimations()
...
def update_html_files():
    ...
    for path in html_files:
        ...
        # For local homepage, also replace the inline script block
        if 'homepage.html' in path or 'index.html' in path:
